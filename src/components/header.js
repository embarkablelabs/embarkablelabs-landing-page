...
function Header({ siteTitle }) {
	...
	return (
		...
		<nav className={`${isFeatureExpanded || isResourcesExpanded || scroll < 0 ? `bg-white shadow-xl transition-colors-shadow duration-500` : 'bg-transparent transition-colors-shadow duration-500'}
			fixed top-0 z-50 w-screen`}>
			<div className={`flex flex-wrap justify-between px-4 py-4 mx-auto mt-0 ${isExpanded && 'bg-white'} w-full lg:w-4/5`}>
...
*/}
				...
				>
						<div className="block my-4 md:my-0 md:inline-block">
							<SubscribeModal small CTAButtonText={landingPageData[0].titleBlock.CTAButtonText} data={landingPageData[0].CTA} event_id="cta_modal"/>
						</div>
				</div>
			...
			{isFeatureExpanded &&
				<div
					onClick = {() => {toggleFeatureExpansion(!isFeatureExpanded)} }
					onMouseLeave = {() => {toggleFeatureExpansion(!isFeatureExpanded)} }
			...
			{isResourcesExpanded &&
				<div
					onClick = {() => {toggleResourcesExpansion(!isResourcesExpanded)} }
					onMouseLeave = {() => {toggleResourcesExpansion(!isResourcesExpanded)} }
